document.addEventListener('DOMContentLoaded', () => {
        ...
        class Particle {
            ...
            draw() {
                ...
                ctx.fillStyle = this.red
                    ? `rgba(215,154,59,${this.a * 0.95})`
                    : `rgba(0,0,0,${this.a})`;
                ctx.fill();
                if (this.glow) {
                    ctx.beginPath();
                    ctx.arc(this.x, this.y, this.r * 3.2, 0, Math.PI * 2);
                    ctx.fillStyle = `rgba(215,154,59,${this.a * 0.16})`;
                    ctx.fill();
                }
            }
        }
    ...
    if (form) {
        form.addEventListener('submit', e => {
            ...
            let txt = `Hola Chocord, soy *${name}*.\n`;
            txt += `\nEstoy interesado/a en: *${interest}*`;
            if (phone)   txt += `\nMi teléfono: ${phone}`;
            if (message) txt += `\n\n${message}`;
            txt += `\n\n¿Podrían brindarme más información, precios y disponibilidad? 😊`;
            ...
            setTimeout(() => {
                window.open(`https://wa.me/529931358045?text=${encodeURIComponent(txt)}`, '_blank');
                form.reset();
...
function initTypewriter() {
    ...
    if (!el) return;
    // All words make grammatical sense: "que [word] tu Paladar"
    const words = ['Enamoran', 'Despiertan', 'Conquistan', 'Endulzan', 'Transforman', 'Inspiran'];
